...
let EditOrder = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8070/order/awaitingpayments/` + "Pending")
      .then((res) => {
        setItem(res.data);
  ...
  const updateOrder = () => {
    ...
    axios
      .put("http://localhost:8070/order/update/" + item._id, data)
...
class AwaitingPayments extends Component {
  ...
  async componentDidMount() {
    const response = await fetch(
      `http://localhost:8070/order/awaitingpayments/` + "Pending"
    );
                  ...
                  <tr>
                    ...
                    <td>
                      <button type="button" class="btn btn-success">
                        <Link
                          to={"/chnagePaymentStatus/" + order._id}
